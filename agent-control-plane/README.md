...
| CI case | 本次操作 / 输入 | 应看到的效果 |
|---|---|---|
| `safe` | 正常运行：查询 + 发一封邮件 | **PASS / 0** |
| `authority-deny` | `DEMO_VIOLATION=1`：额外删除客户 | **BLOCK / 2**，权限 DENY |
| `incident-fixed` | 历史有重复邮件，本次只发一封 | **PASS / 0**，修复有效 |
| `incident-regression` | `DEMO_REGRESSION=1`：本次发两封 | **BLOCK / 2**，虽有发信权限，仍违反“最多一次” |
| `approval` | workflow 注入未审阅的 `update_customer` | **APPROVAL / 3**，CI 阻断 |
| `missing-trace` | workflow 删除本次 trace | **ERROR / 4**，不把历史证据当通过 |
| `malformed-trace` | workflow 增加损坏 JSON | **ERROR / 4**，不忽略坏文件 |
| `empty-trace` | workflow 将本次 trace 改成 `[]` | **ERROR / 4**，没有行为证据 |
